...
public class ZombieSpawner : MonoBehaviourPun, IPunObservable {
    ...
    private int iZombieCount = 0;//남은 좀비 수
    private int iWave;//현재 웨이브
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info) 
    {
        ...
        if (stream.IsWriting)
        {
            ...
            stream.SendNext(iWave);
        }
        else
        {
            //리모트 오브젝트라면 읽기 부분이 실행됨
            //남은 좀비 수를 네트워크를 통해 받기
            iZombieCount = (int) stream.ReceiveNext();
            //현재 웨이브를 네트워크를 통해 받기 
            iWave = (int) stream.ReceiveNext();
        }
    }

    private void Awake() 
    {
        //Color 타입 변수를 RPC로 전송가능하도록 PUN에등록 
    ...
    private void Update() 
    {
        ...
        {
            //게임 오버시 좀비 미생성
            if (GameManager.instance != null && GameManager.instance.bIsGameover)
            {
                return;
            }
    ...
    private void UpdateUI()//웨이브 정보를 UI로 업데이트
    {
        if (PhotonNetwork.IsMasterClient)
        {
            //호스트는 직접 갱신한 좀비 리스트를 통해 남은 좀비의 수를 표시함
            UIManager.instance.UpdateWaveText(iWave, zombies.Count);
        }
        else
        {
            //클라이언트는 좀비 리스트를 갱신할 수 없으므로, 호스트가 보내준 zombieCount를 통해 좀비의 수를 표시함
            UIManager.instance.UpdateWaveText(iWave, iZombieCount);
        }
    }

    
    private void SpawnWave()//현재 웨이브에 맞춰 좀비를 생성
    {
        //웨이브 1 증가
        iWave++;

        //현재 웨이브 * 1.5에 반올림 한 개수 만큼 좀비를 생성
        int spawnCount = Mathf.RoundToInt(iWave * 1.5f);
    ...
    private void CreateZombie()//좀비 생성
    {
        //사용할 좀비 데이터 랜덤 결정
        ...
        // 생성한 좀비의 능력치 설정
        zombie.photonView.RPC("Setup", RpcTarget.All, zombieData.fHealth, zombieData.fDamage, zombieData.fSpeed, zombieData.skinColor);

        // 생성된 좀비를 리스트에 추가
        zombies.Add(zombie);

        //좀비의 onDeath 이벤트에 익명 메서드 등록
        zombie.onDeath += () => zombies.Remove(zombie);//사망한 좀비 리스트에서 제거
        zombie.onDeath += () => StartCoroutine(DestroyAfter(zombie.gameObject, 10f));//사망한 좀비 10 초 뒤에 파괴
